...
class HomeController: UICollectionViewController, UICollectionViewDelegateFlowLayout, HomePostCellDelegate {
    ...
    fileprivate func fetchPostsWithUser(user: User) { // for user, not with
    ...
    func didTapLike(for cell: HomePostCell) {
        ...
        let values1 = ["postId": 1]
        Database.database().reference().child("liked_posts").child(uid).child(postId).updateChildValues(values1)
